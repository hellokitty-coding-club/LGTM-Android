...
class SduiAdapter(private val role: Role) :
    ListAdapter<SduiItemVO, SduiBaseHolder>(ItemDiffCallback<SduiItemVO>(onContentsTheSame = { old, new -> old == new },
    ...
    override fun onBindViewHolder(holder: SduiBaseHolder, position: Int) {
        holder.bind(
            theme = getItem(position).theme, viewContent = getItem(position).content, role = role
        )
    }
